...
with open(output_file, 'w') as output_file:
	...
	s += '<instance xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="src/frodo2/algorithms/XCSPschemaJaCoP.xsd">\n'
	s += '\t<presentation name="{}" maxConstraintArity="3" maximize="false" format="XCSP 2.1_FRODO">\n'.format(scen)
	s += '\t</presentation>\n'
	s += '\t<agents nbAgents="{}">\n'.format(nb_agent)
	...
	for i in range(len(dom_tab2)):
		s += '\t\t<domain name="{}" nbValues="{}">{}</domain>\n'.format(dom_tab2[i][0], len(dom_tab2[i]), ' '.join(dom_tab2[i][1:]))
